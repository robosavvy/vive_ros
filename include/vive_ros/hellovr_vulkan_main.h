#ifndef _HELLOVR_VULKAN_H_
#define _HELLOVR_VULKAN_H_
...
#include "../samples/shared/lodepng.h"
...
bool CMainApplication::CreateAllShaders()
{
	...
	for ( int32_t nShader = 0; nShader < PSO_COUNT; nShader++ )
	{
		for ( int32_t nStage = 0; nStage <= 1; nStage++ )
		{
			...
			sprintf( shaderFileName, "%s/samples/bin/shaders/%s_%s.spv", OPENVR_PATH, pShaderNames[ nShader ], pStageNames[ nStage ] );
...
{
	std::string sExecutableDirectory = Path_StripFilename( Path_GetExecutablePath() );
  std::string strFullPath = Path_MakeAbsolute( std::string(OPENVR_PATH) + "/samples/bin/cube_texture.png", sExecutableDirectory );
	...
	test_ptr = &m_pSceneStagingBuffer;
